...
class VendorShotItemWidget(ShotItemWidget, object):
    ...
    def add_exec(self, from_btn=False):
        ...
        global_pkg_dir_template = get_path(self.job, 'global_pkg_dir')
        global_pkg_dir = global_pkg_dir_template.format(self.base_data)
        for file_data in self.base_data.get('files'):
            source_path = pathlib.Path(file_data['source_path'])
            source_name = source_path.name
            destination_path = pathlib.PurePath(global_pkg_dir, source_name)
    ...
    def get_fi_data(self):
        fi_widget = gui_file_importer.FileImporterWidget(current_pkg_dir=self.pkg_dir, pkg_for=self.pkg_for,
                                                         show_data=None)
        fi_widget.show()
        fi_widget.exec_()
...
if __name__ == '__main__':
    ...
    global_pkg_data = {'date': '20221214', 'vendor': 'dasein', 'show': 'marinet', 'pkg_version_prefix': 'v',
                       'pkg_version_num': '0004',
                       'pkg_dir': '/mnt/mpcparis/MARINET/io/to_shakti/packages', 'pkg_for': 'vendor',
                       'vendor_name': 'Shakti'}

    w = VendorShotItemWidget(_global_pkg_data=global_pkg_data)
    w.show()
    app.exec_()
